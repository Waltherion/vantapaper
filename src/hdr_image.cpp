...
// Shared colour math + row-parallel helper (also used by the video decoder).
using colormath::parallelRows;
...
// Colour math now lives in color_math.h, shared with the video decoder (the GPU
// video-convert shader replicates the same formulas). Aliased here so the decoder
// bodies below stay unchanged.
using colormath::pqEotf;
using colormath::hlgSceneLinear;
using colormath::srgbEotf;
using colormath::bt2020ToBt709;
using Tf = colormath::Tf;
using colormath::linChan;
